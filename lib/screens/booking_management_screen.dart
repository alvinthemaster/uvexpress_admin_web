          ...
          child: Column(
            ...
            children: [
              ...
              _buildProofOfPaymentImage(
                widget.booking!.proofOfPaymentBase64,
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              OutlinedButton.icon(
                onPressed: () => _downloadBase64Image(base64Data, 'booking_proof_of_payment'),
...
class _ManualBookingDialogState extends State<_ManualBookingDialog> {
  ...
  Future<void> _loadReservedSeats() async {
    ...
    try {
      ...
      for (var doc in bookingSnapshot.docs) {
        try {
          ...
          if (booking.bookingStatus == 'confirmed' || booking.bookingStatus == 'active' || booking.bookingStatus == 'verified') {
            ...
            // CRITICAL: Check if booking is for the same van plate number
            bool isSameVan = booking.vanPlateNumber == _selectedVan!.plateNumber;
            
            if (isSameDay && isSameVan) {
              reserved.addAll(booking.seatIds);
              print('  ✓ Booking ${doc.id}: Van ${booking.vanPlateNumber}, Seats: ${booking.seatIds.join(", ")} - ${booking.bookingStatus}');
            }
          }
        } catch (e) {
      ...
      print('✅ Reserved seats loaded for van ${_selectedVan!.plateNumber}: ${reserved.isEmpty ? "None (All available)" : reserved.join(", ")}');
    } catch (e) {
  ...
  Future<void> _submitBooking() async {
    ...
    try {
      ...
      final String autoEmail = '${_phoneController.text.trim()}@admin.booking';
          ...
          children: [
            ...
            Expanded(
              child: Stepper(
                ...
                steps: [
                  Step(
                    title: const Text('Select Current Boarding Vehicle'),
  ...
  Widget _buildVanSelection() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Select Current Boarding Vehicle', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        const SizedBox(height: 8),
        const Text(
          'Choose a vehicle (van or bus) that is currently boarding passengers',
                ...
                return GestureDetector(
                  onTap: () async {
                    ...
                    setState(() {
                      ...
                      // Auto-switch layout toggle based on vehicle type
                      _isBusLayout = van.vehicleType.toLowerCase() == 'bus';
                    });

                    print('🚐 Selected ${van.vehicleType}: ${van.plateNumber} (Status: ${van.status}, Available: $availableSeats seats)');
                    print('📐 Layout auto-switched to: ${_isBusLayout ? "Bus (22 seats)" : "Van (18 seats)"}');
            ...
            child: Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 6),
                  Text(
                    'Layout auto-switched to ${_isBusLayout ? "Bus" : "Van"} based on selected vehicle',
                    style: TextStyle(
                      fontSize: 11,
                      color: Colors.green[900],
